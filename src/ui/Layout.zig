...
pub fn initSplitLayout(
    ...
    const layout1_ptr = try alloc.create(This);
    errdefer alloc.destroy(layout1_ptr);
    const layout2_ptr = try alloc.create(This);
...
pub fn initBorderLayout(child: This, border_width: i32, alloc: Allocator) !This {
    const child_ptr = try alloc.create(This);
    errdefer alloc.destroy(child_ptr);
    child_ptr.* = child;
    return This{ .content = .{ .border = .{
        .alloc = alloc,
        .child = child_ptr,
...
fn recalculateSplitLayout(this: *This) void {
    ...
    while (current_size < wanted_size) {
        ...
        if (layout1_undersized and !layout2_undersized)
            ...
            layout2_size += 1
        else if (layout1_size * split.layout2_desired_size_ratio < layout2_size * split.layout1_desired_size_ratio)
            layout1_size += 1
        else
            layout2_size += 1;
    }
...
fn handleSplitBarUpdate(this: *This) void {
    ...
    if (this.isPointInLayout(mx, my) and !split.layout1.isPointInLayout(mx, my) and !split.layout2.isPointInLayout(mx, my)) {
    ...
    switch (this.content) {
        ...
        .split => |split| {
            ...
            if (split.split_direction == .horizontal) {
                ray.DrawRectangle(this.x, split.layout2.y - split.split_bar_width, this.width, split.split_bar_width, bar_color);
            } else {
                ray.DrawRectangle(split.layout2.x - split.split_bar_width, this.y, split.split_bar_width, this.height, bar_color);
            }
        },
        .border => |border| {
            border.child.render();
